...
def main():
    ...
    with tab2:
        ...
        while st.session_state.get("running", False):
            ...
            df["Tijd"] = pd.to_datetime(df["Tijd"], format="%H:%M:%S")
            recent = df[df["Tijd"] >= df["Tijd"].max() - pd.Timedelta(seconds=10)]

            if not recent.empty:
                # Maak drie kolommen voor de grafieken
                col1, col2, col3 = st.columns(3)

                # Temperatuur grafiek (fig1)
                with col1:
                    fig1 = px.line(recent, x="Tijd", y=["Temperature 1 (remote) [°C]", "Temperature 2 (sensor) [°C]"],
                                   color="Actuator", title="Temperaturen")
                    fig1.update_layout(
                        xaxis=dict(tickformat="%H:%M:%S"),
                        width=500,  # Breedte aanpassen voor betere verdeling
                        height=200,
                        margin=dict(t=40, b=40, l=40, r=10),  # Rechtermarge verkleinen
                        font=dict(size=12)
                    )
                    chart1.plotly_chart(fig1)

                # Delta T grafiek met grenzen (fig2)
                with col2:
                ...
                # Wacht een korte tijd om nieuwe data op te halen
                time.sleep(2)
    ...
    with tab3:
        ...
        start_time = pd.to_datetime(f"{start_hour:02}:{start_min:02}:{start_sec:02}", format="%H:%M:%S")
        end_time = pd.to_datetime(f"{end_hour:02}:{end_min:02}:{end_sec:02}", format="%H:%M:%S")

        st.session_state["data_log"]["Tijd"] = pd.to_datetime(st.session_state["data_log"]["Tijd"], format="%H:%M:%S")
